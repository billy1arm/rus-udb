...
UPDATE `locales_achievement_reward` SET `subject_loc8` = 'Ты побывал везде!' WHERE `entry` = 45;
UPDATE `locales_achievement_reward` SET `text_loc8` = 'Хорошо, посмотрим!
...
UPDATE `locales_achievement_reward` SET `text_loc8` = 'Война бушует повсюду в наших землях. Только самые храбрые герои смеют бросить вызов Орде на границе, где это представляет большую угрозу. Вы в числе таких же героев.
...
UPDATE `locales_achievement_reward` SET `text_loc8` = 'Во время большой суматохи истинные герои поднимаются из нищеты. Вы - единственный великий герой.
...
UPDATE `locales_achievement_reward` SET `subject_loc8` = 'Весьма Успевающий Ученик' WHERE `entry` = 1021;
UPDATE `locales_achievement_reward` SET `text_loc8` = 'Я не мог не заметить ту прекрасную коллекцию накидок, которую вам удалось собрать за месяцы. Вы могли бы также добавить эту к своей коллекции. Она просто собирала пыль в моем платяном шкафу.' WHERE `entry` = 1021;
UPDATE `locales_achievement_reward` SET `text_loc8` = 'Говорят, ты очень заботливо относишься к братьям нашим меньшим. Надеюсь, ты не откажешься приютить у себя моего Вонючку? Он отказывается играть со всеми, кроме тебя...

Пожалуйста, не забывай кормить его дважды в день. И еще... держи его подальше от черных котов.

Бреанни' WHERE `entry` = 1250;
UPDATE `locales_achievement_reward` SET `subject_loc8` = 'Поздравления от Дарнасса' WHERE `entry` = 1681;
UPDATE `locales_achievement_reward` SET `text_loc8` = 'Ваши достижения были основательными и имеющие большие перспективы. Азерот, со всей недавней суматохой, извлекает большую пользу из тех, кто стремится избавить землю от зла.
...
UPDATE `locales_achievement_reward` SET `text_loc8` = 'Аплодисменты и поздравления относительно сбора полного набора монет Даларана! В качестве награды за всю вашу тяжелую работу я приложил недавно отчеканенную Титановую Печать Даларана. Это - специальная монета, которую мы предоставляем только самым горячим из коллекционеров.
